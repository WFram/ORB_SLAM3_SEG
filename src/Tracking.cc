...
Sophus::SE3f Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename)
{
    ...
    if (mSensor == System::STEREO && !mpCamera2)
        mCurrentFrame = Frame(mImGray,imGrayRight,mpSegment->mImgSegmentLatest,timestamp,mpORBextractorLeft,mpORBextractorRight,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera);
    else if(mSensor == System::STEREO && mpCamera2)
        mCurrentFrame = Frame(mImGray,imGrayRight,mpSegment->mImgSegmentLatest,timestamp,mpORBextractorLeft,mpORBextractorRight,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,mpCamera2,mTlr);
    else if(mSensor == System::IMU_STEREO && !mpCamera2)
        mCurrentFrame = Frame(mImGray,imGrayRight,mpSegment->mImgSegmentLatest,timestamp,mpORBextractorLeft,mpORBextractorRight,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,&mLastFrame,*mpImuCalib);
    else if(mSensor == System::IMU_STEREO && mpCamera2)
        mCurrentFrame = Frame(mImGray,imGrayRight,mpSegment->mImgSegmentLatest,timestamp,mpORBextractorLeft,mpORBextractorRight,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,mpCamera2,mTlr,&mLastFrame,*mpImuCalib);
...
Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp, string filename)
{
    ...
    if (mSensor == System::RGBD)
        mCurrentFrame = Frame(mImGray,imDepth,mpSegment->mImgSegmentLatest,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera);
    else if(mSensor == System::IMU_RGBD)
        mCurrentFrame = Frame(mImGray,imDepth,mpSegment->mImgSegmentLatest,timestamp,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,&mLastFrame,*mpImuCalib);
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    // const bool c1b = ((mCurrentFrame.mnId>=mnLastKeyFrameId+mMinFrames) && bDynamicsCullingIdle); //mpLocalMapper->KeyframesInQueue() < 2);
    ...
    if(((c1a||c1b||c1c) && c2)||c3 ||c4)
    {
        ...
        else
        {
            ...
            if(mSensor!=System::MONOCULAR  && mSensor!=System::IMU_MONOCULAR)
            {
                // if(mpDynamicCuller->KeyframesInQueue()<3) // Adjust these parameters
                if(mpLocalMapper->KeyframesInQueue()<3) // Adjust these parameters
                    return true;
                else
                    return false;
            }
